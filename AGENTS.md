# AGENTS.md
## Autonomous & Human Agent Coordination Rules

This project is a **Jitsi Meeting Scheduling & Orchestration Platform**
built on Laravel 12 with Tyro RBAC and Tyro Dashboard.

This file defines how AI agents and humans must behave.
...
If conflicts exist, resolve upward.

---

## 2. Non-Negotiable Rules

- Jitsi is an **external system**
- No scheduling logic is allowed inside Jitsi
- All access control is enforced **server-side**
- Dashboard pages MUST be generated using Tyro Dashboard Artisan commands
- No manual sidebar or admin UI wiring
- Authorization MUST use Tyro privileges
- Meeting rooms are never directly exposed
...
- Must respect DOMAIN_RULES.md
- Must not modify dashboard UI manually

### Dashboard/UI Agent
- Uses Tyro Dashboard generators only
- Edits only generated Blade views
- No custom admin layouts allowed

### Integration Agent
- Treats Jitsi as read-only external
- Uses JWT only if enabled
- Must not depend on Jitsi state
...
- Exposing Jitsi room URLs publicly
- Client-side join eligibility checks
- Building custom admin dashboards outside Tyro Dashboard
...
A build is considered valid if:
- Meetings cannot be joined early
- Dashboard pages exist via Tyro scaffolding
- Access control is enforced via Tyro
- Jitsi access is mediated by backend logic

Failure to meet these invalidates the build.
