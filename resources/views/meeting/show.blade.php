<!DOCTYPE html>
@php $isMember = auth()->check() && auth()->user()->hasRole('member'); @endphp
<html lang="en">
<head>
    <meta charset="UTF-8">
    ...
    <header class="top-header">
        <div class="header-container">
            ...
            <div class="header-actions">
                @auth
                    <a href="{{ $isMember ? route('dashboard.my-meetings') : route('tyro-dashboard.index') }}" class="btn-secondary">
                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" style="width: 18px; height: 18px;">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path>
                        </svg>
                        <span>{{ $isMember ? 'My Meetings' : 'Dashboard' }}</span>
                    </a>
                @else
        ...
        <div class="meeting-card">
            ...
            <div class="meeting-footer">
                ...
                <div class="footer-links">
                    ...
                    @endguest
                    @auth
                        <a href="{{ $isMember ? route('dashboard.my-meetings') : route('tyro-dashboard.index') }}">{{ $isMember ? 'My Meetings' : 'Dashboard' }}</a>
                    @endauth
                </div>
            </div>
        </div>
            ...
            try {
                ...
                jitsiApi.addEventListener('readyToClose', async () => {
                    ...
                    window.location.href = isAuthenticated
                        ? @json(route('tyro-dashboard.index'))
                        : @json(url('/'));
                });
