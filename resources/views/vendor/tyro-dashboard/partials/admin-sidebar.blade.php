<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <div class="sidebar-section">
            ...
            @php
                $isOrgAdmin = auth()->check()
                    && method_exists(auth()->user(), 'hasRole')
                    && auth()->user()->hasRole('org-admin')
                    && !auth()->user()->hasRole('super-admin');
            @endphp
            @if($isOrgAdmin)
                {{-- Org-admin: organisation-scoped User Management only --}}
                <a href="{{ route('dashboard.team.index') }}" class="sidebar-link {{ request()->routeIs('dashboard.team.*') ? 'active' : '' }}">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
